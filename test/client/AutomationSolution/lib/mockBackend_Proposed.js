...
function MockBackend(configs){
    ...
    proxy.onLoad.whenGET(/api\/objects\/[0-9]+\/notes\/regular/);
    proxy.onLoad.whenPOST('api/licensing/WebClient/login').passThrough();

    //temporary workaround to fix tests failing because of "unexpected request: POST Search/GetAdvancedSearchCriteria"
    proxy.onLoad.whenPOST('Search/GetAdvancedSearchCriteria').passThrough();

    //temporary workaround to fix tests failing on webclient load because of unexpected GetAllTasks/Diaries calls
    proxy.onLoad.whenGET('/Task/GetAllTasks').respond(200);
    proxy.onLoad.whenGET('/Task/GetAllDiaries').respond(200);
